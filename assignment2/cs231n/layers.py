...
def batchnorm_backward(dout, cache):
    ...
    N, x, sample_mean, sample_var, eps, gamma, beta = cache
...
def batchnorm_backward_alt(dout, cache):
    ...
    #dx = dout * gamma * (var + eps) ** (-1./2.) + \
    #     1./N * np.sum(dout * gamma, axis=0) * (var+eps) ** (-1./2.) * -1. + \
    #     2./N * (x - mu) * (var + eps) ** (-3./2.) * np.sum(dout * gamma * (x - mu), axis=0) * (-1./2.)
    #dx = 1./N * gamma * (var + eps) ** (-1./2.) * \
    #     (N * dout - np.sum(dout, axis=0) - ((var + eps) ** (-1.)) * np.sum(dout * (x-mu) , axis=0))

    N, x, sample_mean, sample_var, eps, gamma, beta = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum((x - sample_mean) * (sample_var + eps) ** (-1./2.) * dout, axis=0)
    dx = (1./N) * gamma * (sample_var + eps) ** (-1./2) * \
         (N * dout - np.sum(dout, axis=0) - (x - sample_mean) * (sample_var + eps) ** (-1.0) * np.sum(dout * (x - sample_mean), axis=0))
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for f in range(C):
            for j in range(H_new):
                for k in range(W_new):
                    windows = x[i, f, j*stride:j*stride+pool_height, k*stride:k*stride+pool_width]
                    out[i, f, j, k] = np.max(windows)
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for f in range(C):
            for j in range(H_new):
                for k in range(W_new):
                    windows = x[i, f, j*stride:j*stride+pool_height, k*stride:k*stride+pool_width]
                    m = np.max(windows)
                    dx[i, f, j*stride:j*stride+pool_height, k*stride:k*stride+pool_width] = (windows == m) * dout[i, f, j, k]
    pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
